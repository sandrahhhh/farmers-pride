...
if(isset($_POST["getProduct"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			<div id='card'>
			<a href='productDetails.php?pid=$pro_id'>
			<img src='product_images/$pro_image'>
			</a>
			<br><br>
			<p>$pro_title</p>
			<p>ZMW $pro_price.00/day</p>
			<button style='float:right;' class='tn  btn-success-custom btn-xs' data-toggle='modal' data-target='#addProduct' pid='$pro_id' id='product'>Add To Cart</button>
			</div>";
		}
	}
}
		...
		if (mysqli_num_rows($query) > 0) {
			...
			echo "<form method='post' action='login_form.php'>";
				...
				while ($row=mysqli_fetch_array($query)) {
					...
					echo 
						'<div class="row row-custom">
								...
								<div class="col-md-2"><input type="number" min="0" class="form-control qty" value="'.$qty.'" required></div>
								<div class="col-md-2"><input type="text" class="form-control price" value="'.$product_price.'" readonly="readonly"></div>
								<div class="col-md-2"><input type="number" min="0" class="form-control days" value="'.$qty.'" required></div>
								<input type="hidden" class="form-control total" value="'.$product_price.'" readonly="readonly">
							</div>';
				}
				...
				if (!isset($_SESSION["uid"])) {
					...
					//Paypal checkout form <a href="mobile_money/paymentForm.php">mobile money</a>
					echo '
						</form>
