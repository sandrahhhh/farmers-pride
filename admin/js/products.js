$(document).ready(function(){
					...
					if (productList) {
						$.each(resp.message.products, function(index, value){
							...
							productHTML += '<tr>'+
								              ...
								              '<td><a class="btn btn-sm btn-info edit-product" style="color:#fff;">Edit<span style="display:none;">'+JSON.stringify(value)+'</span><i class="fas fa-pencil-alt"></i></a>&nbsp;<a pid="'+value.product_id+'" class="btn btn-sm btn-danger delete-product" style="color:#fff;">Delete<i class="fas fa-trash-alt"></i></a></td>'+
								            '</tr>';
	...
	$(document.body).on("click", ".add-product", function(){
		$.ajax({
			url : '../admin/classes/Products.php',
			method : 'POST',
	...
	$(document.body).on('click', '.edit-product', function(){
		...
		$("select[name='e_status']").val(product.status);
